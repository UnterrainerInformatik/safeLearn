...
| `test/deployment-config.test.js` | That every environment variable the application reads reaches a deployed instance — through the image's own `.env` or through the container's environment — and that neither path is wired up only halfway. No browser, no login, no server: it reads the workflow, the compose file and the application's own sources. |
...
```
A separate run for the companion Obsidian plugin, which gives this project's tags a representation while a teacher writes them. It starts no server. It does open a real Obsidian window — there is no headless mode for an Electron application without a virtual display, and requiring one to run a check would be worse than a window that appears for ten seconds.

All but two of its checks reach every state through the plugin's own facts and need no realm, no account and no network. The two that cannot are the change `plugin-login-flow`'s: what the realm answers for a refresh token's lifetime is the figure a login in progress is given to conclude in, and a login that completes from one end to the other is the one thing a stubbed access token cannot stand in for. Both go to the same identity provider and the same teacher account the run below uses, through the same `SAFELEARN_TEST_TEACHER_USER` / `SAFELEARN_TEST_TEACHER_PASSWORD`, so one installation is still configured once. The practical consequence is that this run no longer passes with no network; it fails naming what it looked for rather than skipping, because a check that cannot reach a state and reports a pass is worse than one that is red.
...
### What `test/obsidian/` checks of the plugin's own PKCE login, and what is still by hand

`plugin-admin-directory-ui` gave the plugin its own Authorization Code + PKCE login against Keycloak, completed in the system's default browser and returned through Obsidian's `obsidian://safelearn-formatter-auth` protocol handler. Everything reachable *without* a live login — the settings tab, the search strip and `list-classes` staying gated behind `hasLogin()`, the search/pick/append flow, the class-detection heuristic — is exercised in `test/obsidian/plugin.test.js` against a plugin instance whose directory client and access token are stubbed directly (`setDirectoryLoginFixture`).

The round trip is too, since `plugin-login-flow`. This page used to say it could not be, and half of that was right: driving a real *OS-level browser* through an interactive Keycloak login is outside a CDP-attached Puppeteer session, and still is. But the round trip does not need one. The realm's login page is an HTML form and its answer is a `Location` header, so `completeRealLogin` walks both in Node's `fetch` and hands the authorization code back through `app.workspace.protocolHandler.dispatch` — the same path Obsidian's own `obsidian://` handling takes. The plugin starts the login, so the verifier, the challenge and the `state` are its own, and the exchange and everything written to `data.json` stay the plugin's: the harness delivers a browser's answer, it does not stand in for the plugin.

What is still by hand is the part that is genuinely about the operating system: that clicking **Log in** opens *the person's own default browser*, and that the OS hands `obsidian://…` back to the right application. That, and the platform coverage below, is checked once per platform, against a safeLearn instance whose realm carries the `safelearn-plugin` client (`tasks.md` §1 — an operator-provisioned, one-time step; nothing about it is automatable, and it has to exist before any of the steps below can be tried at all):
...
| `render(session, path, { view })` | Navigates to `path` as `page`, `presentation` or `document` and returns `{ page, text, url }` once that view's owner has **shown** it — not merely once the content is in the DOM. Every view is served hidden, and until it is shown every element on it measures zero by zero, so the page this hands back is one a check can click, type into and measure. Do not wait for `document.body.style.display` after calling it. Throws — naming the path, the view and where the request ended up — if the application redirected instead, if the content never appears, or if it appears and the page is never shown. |
...
| `… rendered but was never shown …: the body stayed hidden` | The view's owner never cleared `display: none`. For a page or a document that is `revealPage()` in `obsidian-page.js`, so `/userattributes` did not answer and its five-second bound did not fire either — usually a script error earlier in the page. For a presentation it is the deck's own reveal script. The failure names which of the two it was waiting for; the browser console says the rest. |
