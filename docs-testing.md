...
| `test/checks/permissions.js` | Block-level and whole-file `@@@` directives in both directions, their effect on the navigation tree, time-bounded windows, and the teacher-to-student downgrade. |
...
* **`md/test-md-file.md` — a `@@@ admin` block is visible to everyone.** `hasRoles` in `utils.js` computes `isAdmin = r.admin || normalizedClientRoles.includes("admin")`, so the *requested* role satisfies the check by itself and any session sees the block. Both demo accounts see "Admins only!!!". There is no way to assert the intended behavior without changing that line, so no check asserts on this block at all.
