...
| GET | `/api/admin/directory/search?q=<wildcard>` | bearer token, introspected | Registered ahead of `checkAuthenticated` and never added to its chain: the caller is a teacher or admin proven by `Authorization: Bearer <token>`, checked against Keycloak's introspection endpoint (`middlewares/directory-service.js`), not by the browser session. Matches `q` case-insensitively against a directory user's display name or any role/group they hold — merged from their `ldap` attribute's `OU=` parts and their `safeLearn` client roles, the same two sources `hasRoles` treats as interchangeable for a session — using a second, dedicated Keycloak client (`DIRECTORY_SERVICE_CLIENT_ID`/`_SECRET`, `view-users` only) to query the Admin REST API — never the caller's own token. Returns `[{ name, roles }, ...]`; an empty or missing `q` returns the whole directory mapped the same way, rather than `[]` — added for `plugin-admin-directory-ui`'s "list classes", which has no substring guaranteed to match everyone. The realm's full user list, including a role-mappings call per user, is cached in-process for 10 minutes rather than fetched per search |
...
- **`/api/admin/directory/search` is also registered ahead of `checkAuthenticated`, for the opposite reason.** Added by the change `plugin-admin-api-backend`: `/hot-reload` sits there because it needs no identity at all, while this route sits there because it needs a *different* one — a bearer token proven fresh by Keycloak introspection, independent of the session cookie every other route trusts. Joining the `checkAuthenticated` chain would have meant answering to a session that happens to be present as well as, rather than instead of, the token; the route checks the token alone and ignores any cookie a request carries. It talks to Keycloak under two identities that never mix: the existing `safeLearn` client's own credentials to introspect the caller's token, and a second, `view-users`-only service-account client (`DIRECTORY_SERVICE_CLIENT_ID`/`_SECRET`) to query the Admin REST API — the calling teacher's or admin's own Keycloak access, whatever it is, is never used for the lookup itself. `docs-keycloak.md` names what the second client needs, and `test/checks/deployment-surface.js` and `test/checks/directory-search.js` assert both halves.
- **The shared demo realm this project tests against has no LDAP federation configured at all.** Found while implementing `plugin-admin-api-backend`: every one of its accounts' `attributes.LDAP_ENTRY_DN` is empty (verified against the live Admin API), and a logged-in session's `req.user.ldap` and `getLdapGroups(req)` are likewise always empty — `teacher`, `admin`, classes and the `examParticipant` marker are all granted on this realm as Keycloak client roles on `safeLearn` directly, not through the `ldap` claim `docs-keycloak.md` describes. `utils.js`'s `hasRoles` already merges both sources for a session, so this has always been true of role checks generally; it only became visible while building directory search's role/group data, which reads `attributes.LDAP_ENTRY_DN` for a user who is not the current session and found nothing there. `directoryUserRoles` (`middlewares/directory-service.js`) now merges the same two sources per directory user. A real, LDAP-federated deployment (per `docs-keycloak.md`) would populate the first source instead of or alongside the second.
...
- **The plugin gained its own identity toward Keycloak, independent of the server's browser session.** Added by `plugin-admin-directory-ui`: a full Authorization Code + PKCE login against the school's Keycloak realm, completed in the system browser and returned through Obsidian's own `obsidian://<manifest-id>-auth` protocol handler rather than a loopback HTTP listener — the design rejects a loopback listener outright, since it needs a real TCP server that Obsidian mobile does not have. A new `PluginSettingTab` (the plugin had none before this) carries three settings: the safeLearn instance URL (empty by default, and everything below stays off without it), and a Keycloak URL / Realm pair defaulted to this project's shared identity provider — needed because `keycloak.json` is deliberately never served to a browser (`test/checks/deployment-surface.js`), so the plugin cannot discover its own realm's connection details remotely. Only the refresh token is persisted (`saveData`/`data.json`); the access token lives in memory only and is re-derived on load and before each directory call. The new Keycloak client this needs (`safelearn-plugin`, public, PKCE-only, no client roles of its own) is an operator-provisioned step documented in the plugin's own `README.md`, not something the plugin or a person installing it can do.
- **`GET /api/admin/directory/search` gained a UI on the plugin side, and one small server change to make "list classes" possible at all.** The "List classes" command and the directory picker `NameListModal` gained above its textarea (both gated behind a login, silently absent without one) share one fetch: every role/group value across the whole directory that is not one of `teacher`, `teachers`, `student`, `students`, `admin` — an accepted approximation, since the backend has no closed notion of "class" for the plugin to recover (an `examParticipant`-style marker is listed exactly like an actual class). That fetch needs an empty query to enumerate everyone, which the endpoint originally answered with `[]` — discovered mid-implementation, and resolved by having an empty or missing `q` return the whole directory instead (see the route table above and `specs/directory-search/spec.md` in the change). Nothing else about the endpoint changed.
- **The plugin's real PKCE round trip is not covered by `test/obsidian/`, and cannot be from this harness.** Driving a real OS-level browser through an interactive Keycloak login, and an Electron app's custom URI scheme dispatch delivering the callback back to it, is outside what a CDP-attached Puppeteer session can do — `plugin-verification`'s own harness starts no browser of its own and authenticates against nothing, by design. Everything reachable without a live login (the settings tab, the gating of the search strip and `list-classes` behind `hasLogin()`, the search/pick/append flow, the class-detection heuristic) is exercised in `test/obsidian/plugin.test.js` against a plugin instance whose directory client and access token are stubbed directly on the running instance (`setDirectoryLoginFixture` in `test/obsidian/harness.js`) rather than through a real Keycloak exchange. The login itself is verified by hand instead — `docs-testing.md` records what that check covers and what it does not.
- **`executeCommandById` does not consult `checkCallback`'s return value at all.** Found while writing the check above: Obsidian's command dispatch calls a command's action directly and reports success unless it throws, regardless of what `checkCallback` returns — confirmed empirically by running `list-classes` through it with `checkCallback` forced to return `false`. `checkCallback(true)` ("checking, don't run") is what the command palette and the hotkey manager actually call to decide whether to offer a command; a check about a command being "not shown" has to ask that directly (`commandIsAvailable` in `test/obsidian/harness.js`), not whether a raw `executeCommandById` call was accepted.
