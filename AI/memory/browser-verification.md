...
**Why:** Jeder Lauf authentifiziert sich **live gegen `https://auth.unterrainer.info`** mit den geteilten öffentlichen Demo-Accounts `student`/`student`, `teacher`/`teacher` und `admin`/`admin`. Ohne Netz schlägt der Lauf fehl, einen Offline- oder Mock-Modus gibt es nicht. SafeLearn legt Preferences (`config`, `lastVisitedUrl`) als Keycloak-User-Attribute ab — ein Testlauf verändert also serverseitigen Zustand eines Accounts, den andere gleichzeitig benutzen können.
